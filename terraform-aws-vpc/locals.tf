locals {
  ...
  vpc_final_tags = merge(
    ...
    availability_zones_names = slice(data.aws_availability_zones.available.names, 0, 2)
         # this is for getting the names of the availability zones in the current region, we are using the aws_availability_zones data source to get the availability zones and then we are using the names attribute to get the names of the availability zones and assigning it to the local variable availability_zones_names, so that we can use this variable in other terraform file by calling this module.
    public_subnet_tags = merge(
        local.common_tags,
         #roboshop-dev-public-east-1a,reboshop-dev-public-east-1b
        {
            Name = "${var.project}-${var.environment}-public-${local.availability_zones_names[0]}"  #this is for giving the name to our public subnets, we are using the merge function to merge the common_tags with the name tag, so that we can have the name tag in our public subnets along with the common tags
        }
    )
