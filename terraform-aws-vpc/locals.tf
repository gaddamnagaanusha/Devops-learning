...
#     {
#         Name = "${var.project}-${var.environment}-private-${local.private_availability_zones_names[0]}"  #this is for giving the name to our private subnets, we are using the merge function to merge the common_tags with the name tag, so that we can have the name tag in our private subnets along with the common tags
#     }
# )
